...
/**
 * 可展开/折叠的TextView
 * 核心方法：[createStaticLayout] [onTouchEvent]
 *
 * @see setExpandableText
 * @auther hy
 * create on 2021/09/02 下午5:08
 */
class ExpandableTextView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
    ...
    private var mOriginHeight: Int = 0  // 源文本的高度
    ...
    private var mFoldedSuffix: SpannableString //折叠状态下的文本后缀
    private var mExpandedSuffix: SpannableString//展开状态下的文本后缀

    private var isDefaultFolded: Boolean = true
    ...
    init {
        ...
        isDefaultFolded = typedValue.getBoolean(R.styleable.ExpandableTextView_isDefaultFolded, true)
        mDuration = typedValue.getInt(R.styleable.ExpandableTextView_expandDuration, DEFAULT_DURATION_TIME).toLong()
        mFoldedLines = typedValue.getInt(R.styleable.ExpandableTextView_foldLines, DEFAULT_EXPANDABLE_LINES)
        mSuffixTextColor = typedValue.getColor(R.styleable.ExpandableTextView_suffixTextColor, DEFAULT_SUFFIX_TEXT_COLOR)
        val foldSuffix = typedValue.getString(R.styleable.ExpandableTextView_foldedSuffix)
            ?: DEFAULT_ACTION_TEXT_EXPAND
        mFoldedSuffix = createClickedSpannableString(ELLIPSIS_STRING + foldSuffix, ELLIPSIS_STRING.length)
    ...
    fun setIsDefaultFolded(isFolded: Boolean) {
        isDefaultFolded = isFolded
        mOriginText?.let { resetText(it, isFolded) }
    }
     ...
     */
    private fun resetText(text: CharSequence, isDefaultFolded: Boolean = true) {
   //     mOriginText = text
        if (mMeasuredWidth <= 0) return
        val layout = createStaticLayout(text)
        mOriginHeight = layout.height
        canFold = layout.lineCount > mFoldedLines
        isFolded = canFold && isDefaultFolded
        if (canFold) {
            buildExpandableText(layout)
            if (isFolded) {
                layoutHeight = mFoldedHeight
                setText(mFoldedText)
            } else {
                layoutHeight = mExpandedHeight
                setText(mExpandedText)
            }
            //     setText(if (isFolded) mFoldedText else mExpandedText)
        } else {
            layoutHeight = mOriginHeight
            setText(mOriginText)
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if ((mMeasuredWidth == 0 || mMeasuredWidth != measuredWidth) && !isAnimating) {
            ...
            mOriginText?.let { resetText(it, isDefaultFolded) }
        }
    }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        super.onSizeChanged(w, layoutHeight, oldw, oldh)
    }
...
//        mFoldedText = createSpannableStringBuilder(foldText).append(mFoldedSuffix)
    ...
    private fun <T : CharSequence> createStaticLayout(source: T): Layout =
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            ...
            StaticLayout(source, paint, mMeasuredWidth, Layout.Alignment.ALIGN_NORMAL,
                lineSpacingMultiplier, lineSpacingExtra, includeFontPadding)
        }

    private fun createClickedSpannableString(charSequence: CharSequence, start: Int = 0): SpannableString = SpannableString(charSequence).apply {
    ...
    private fun createAnimation(start: Int, end: Int, startCallback: (() -> Unit)?, endCallback: (() -> Unit)?): ObjectAnimator {
