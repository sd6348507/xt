...
import com.roemsoft.equipment.databinding.ActivityCostComSearchBinding
...
class CostComSearchActivity : DataBindingAppCompatActivity() {

    companion object {
        const val RESULT_SELECTED = "result:selected"
        const val ACTION_SEARCH_COST_COM = 10001 // "action:search:com"
    }

    private val dataBinding: ActivityCostComSearchBinding by binding(R.layout.activity_cost_com_search)

    override val viewModel: CostComSearchViewModel by viewModels()
    ...
    override fun setToolTitle() {
        dataBinding.toolbarTitle.text = getString(R.string.label_base_cost_com)
    }
    ...
    override fun setupEvent() {
        ...
        viewModel.select.observe(this) { result ->
            setResult(ACTION_SEARCH_COST_COM, Intent().putExtra(RESULT_SELECTED, result))
            finish()
        }
