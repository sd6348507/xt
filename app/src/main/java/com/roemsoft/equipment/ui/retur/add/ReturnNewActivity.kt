package com.roemsoft.equipment.ui.retur.add
...
import com.roemsoft.equipment.databinding.ActivityReturnNewBinding
...
class ReturnNewActivity : DataBindingAppCompatActivity() {

    private val binding: ActivityReturnNewBinding by binding(R.layout.activity_return_new)

    override val viewModel: ReturnNewViewModel by viewModels()
    ...
    override fun initView() {
        if (App.isPda()) {
            binding.returnScanImg.visibility = View.INVISIBLE

            val scannerUnit = ScannerUnitBroadcast(this).apply {
                scannerResult.observe(this@ReturnNewActivity) { onScannerResult(it) }
            }
        ...
        binding.returnSearchBtn.onSingleClick {
            viewModel.assetsNo.value = binding.returnSearchEt.text.trim().toString()
            viewModel.loadData()
        }

        binding.returnScanImg.onSingleClick {
    ...
    override fun setToolTitle() {
        binding.toolbarTitle.setText(R.string.label_return)
    }
