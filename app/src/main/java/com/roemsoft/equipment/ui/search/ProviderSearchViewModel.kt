...
class ProviderSearchViewModel : BaseViewModel() {

    // 刷新
    private val _refresh = MutableLiveData(false)
    val refresh: LiveData<Boolean> = _refresh

    private val _select = MutableLiveData<ProviderData>()
    val select: LiveData<ProviderData> = _select

    // 源数据
    // 用于搜索过滤
    private var origList = listOf<ProviderData>()

    private val _searchFlow = MutableStateFlow("")

    val adapter = ProviderSearchAdapter().apply {
    ...
    fun setAdapterData(list: List<ProviderData>) {
