...
import com.roemsoft.equipment.databinding.ActivityProviderSearchBinding
...
class ProviderSearchActivity : DataBindingAppCompatActivity() {

    companion object {
        const val RESULT_SELECTED = "result:selected"
        const val ACTION_SEARCH_PROVIDER = 10001 // "action:search:com"
    }

    private val dataBinding: ActivityProviderSearchBinding by binding(R.layout.activity_provider_search)

    override val viewModel: ProviderSearchViewModel by viewModels()
    ...
    override fun setToolTitle() {
        dataBinding.toolbarTitle.text = getString(R.string.label_provider)
    }
    ...
    override fun setupEvent() {
        ...
        viewModel.select.observe(this) { result ->
            setResult(ACTION_SEARCH_PROVIDER, Intent().putExtra(RESULT_SELECTED, result))
            finish()
        }
